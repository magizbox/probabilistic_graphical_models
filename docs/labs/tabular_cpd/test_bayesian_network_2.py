...
df = pd.DataFrame(
    [["true", "true", 0.1],
     ...
     ["false", "true", 0.9],
     ["false", "false", 0.5]],
    columns=["sprinkler", "cloudy", "p"]
)
bn.add_cpd("sprinkler|cloudy", df)
...
class Test_BayesianNetwork(TestCase):
    ...
    def test_lookup_3(self):
        self.assertTrue(np.allclose(0.5, bn.lookup(rain="true")))
        self.assertTrue(np.allclose(0.5, bn.lookup(rain="false")))
